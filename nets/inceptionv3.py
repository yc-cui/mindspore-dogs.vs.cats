...
from mindspore.common.initializer import XavierUniform, HeNormal
...
class BasicConv2d(nn.Cell):
    def __init__(self, in_channel, out_channel, kernel_size, stride=1, pad_mode='same', padding=0, has_bias=False):
        ...
        self.conv = nn.Conv2d(in_channel, out_channel, kernel_size=kernel_size, stride=stride,
                              pad_mode=pad_mode, padding=padding, weight_init=HeNormal(), has_bias=has_bias)
...
class InceptionV3_head(nn.Cell):
    def __init__(self, num_classes=2):
        super(InceptionV3_head, self).__init__()
...
class InceptionV3_backbone(nn.Cell):
    def __init__(self, has_bias=False):
        super(InceptionV3_backbone, self).__init__()
...
class InceptionV3(nn.Cell):
    def __init__(self, num_classes=2, backbone=None, head=None):
        ...
        if backbone is None:
            self.backbone = InceptionV3_backbone()
        else:
            self.backbone = backbone

        if head is None:
            self.head = InceptionV3_head(num_classes)
        else:
            self.head = head
